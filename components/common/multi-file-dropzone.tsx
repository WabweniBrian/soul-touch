...
const variants = {
  base: "relative h-[100px] rounded-md p-4 w-full flex justify-center items-center flex-col cursor-pointer border border-dashed border-border transition-colors duration-200 ease-in-out",
  active: "border-2",
  disabled:
      ...
      <div>
        <div className="flex flex-col gap-2">
          ...
          {value?.map(({ file, progress }, i) => (
            <div
              key={i}
              className="flex h-16 w-full flex-col justify-center rounded border border-border px-4 py-2"
            >
              <div className="flex items-center gap-2 text-gray-500 dark:text-white">
                <FileIcon size="30" className="shrink-0" />
                <div className="min-w-0 text-sm">
                  <div className="overflow-hidden overflow-ellipsis whitespace-nowrap">
                    {trimFileName(file.name, 20)}
                  </div>
